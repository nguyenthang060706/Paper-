...
class FPRBudgetManager:
    """
    Qu?n lÃƒÂ½ FPR Budget d?ng d?a trÃƒÂªn thÃ¡Â»â€˜ng kÃƒÂªÃƒÂª th?c t?.

    FIX v3.6:
    - min_threshold: 0.40 ? 0.58 (trÃƒÂ¡nh block benign cÃƒÂ³ ML score 0.40-œ0.57)
    - deadband: ch? loosen khi fpr_usage_ratio < 0.5, ch? tighten khi > 1.2 (trÃƒÂ¡nh oscillation)
    - warmup_decisions=30: khÃƒÂ´ng n?i l?ng trong 30 quy?t d?nh d?u
    - T?c d? tighten (0.06) g?p dÃƒÂ´i loosen (0.03)
    """
    target_fpr: float = 0.05
    initial_threshold: float = 0.70
    min_threshold: float = 0.58        # FIX v3.6: 0.40 ? 0.58
    # [FIX R5-2] ThÃƒÂªm max_threshold d? tighten cÃƒÂ³ upper bound dÃƒÂº ng.
    # Tru?c d y dÃƒÂ¹ng initial_threshold (0.70) ? khi FPR r?t cao,
    # threshold ch? cÃƒÂ³ th? tang tÃ¡ÂºÂ¡i 0.70 Ã¢â‚¬- khÃƒÂ´ng d? ch?t khi c?n.
    ...
    def adjust_threshold(self) -> float:
        ...
        if fpr_usage_ratio < 0.5 and not in_warmup:
            ...
            # [FIX R5-2] cap ? max_threshold (0.92) thay vÃƒÂ¬ initial_threshold (0.70)
            # d? threshold cÃƒÂ³ th? tighten vu?t quÃƒÂ¡ initial value khi FPR r?t cao.
...
class AdaptiveEscalationManager:
    ...
    FIX v3.6b: FPRBudgetManager yÃƒÂªu c?u actual_label (ground-truth) d? tÃƒÂ­nh FPR Ã¢â‚¬-
    khÃƒÂ´ng kh? thi trong production vÃƒÂ¬ agent khÃƒÂ´ng bi?t tru?c nhÃƒÂ£n th?t.

    AdaptiveEscalationManager thay th? b?ng cÃƒÂ¡ch theo dÃƒÂµi escalation_rate:
    t? l? quy?t d?nh DENY/QUARANTINE trong c?a s? W quy?t d?nh g?n nh?t.
    NgÃ†Â°Ã¡Â»Â¡ng Ã„â€˜Ã†Â°Ã¡Â»Â£c di?u ch?nh d? gi? escalation_rate Ã‹Å“ target_escalation_rate,
    khÃƒÂ´ng c?n nhÃƒÂ£n th?t Ã¢â‚¬- hoÃƒÂ n toÃƒÂ n label-free vÃƒÂ  production-ready.

    Co ch? di?u ch?nh ngÃ†Â°Ã¡Â»Â¡ng:
    - N?u escalation_rate > target * 1.2  ? tighten (tang threshold a?=0.06)
    - N?u escalation_rate < target * 0.5  ? loosen  (gi?m threshold a?=0.03)
    - Warmup guard: khÃƒÂ´ng di?u ch?nh trong N quy?t d?nh d?u tiÃƒÂªn
    """
    target_escalation_rate: float = 0.10  # M?c tiÃƒÂªu: ~10% actions b? escalate
    initial_threshold: float = 0.70
    ...
    # [FIX v3.6c Ã¢â‚¬- Perf] deque(maxlen) vs List.pop(0): O(1) vs O(n) per append.
    ...
    def __post_init__(self):
        ...
        # [FIX v3.6c Ã¢â‚¬- Perf] Replace list with deque for O(1) pop-left.
    ...
    def record_decision(self, is_escalated: bool) -> None:
        """Ghi nhÃ¡ÂºÂ­n m?t quy?t d?nh (khÃƒÂ´ng c?n nhÃƒÂ£n th?t)."""
        self._decisions.append(is_escalated)  # deque auto-drops oldest at maxlen
        self._total_decisions += 1
        # [FIX v3.6c Ã¢â‚¬- Perf] deque(maxlen=window_size) auto-drops oldest;
    ...
    def _adjust_threshold(self) -> None:
        ...
        if ratio > 1.2:
            # QuÃƒÂ¡ nhi?u escalation ? tighten (khÃƒÂ³ escalate hon)
            self._threshold = min(self._threshold + self.tighten_step, self.max_threshold)
        elif ratio < 0.5:
            # QuÃƒÂ¡ ÃƒÂ­t escalation ? loosen  (nh?y c?m hon)
...
class CumulativeRiskScorer:
    """
    ÃƒÂÃƒÂ¡nh giÃƒÂ¡ r?i ro c?ng d?n trong phiÃƒÂªn lÃƒÂ m vi?c.

    Attributes:
        session_id: ÃƒÂ?nh danh phiÃƒÂªn
        base_cumulative_threshold: NgÃ†Â°Ã¡Â»Â¡ng r?i ro t?ng ?_cumul (default: 8.0)
        decay_rate: H? s? suy gi?m tuy?n tÃƒÂ­nh (e.g., 0.1 per hour)
    ...
    [BUG-5 FIX] _apply_decay() now:
        ...
        itself Ã¢â‚¬- base_cumulative_threshold was never enforced.
    """
    session_id: str
    ...
    def add_risk_event(self, action: str, risk_score: float, reason: str,
        ...
        """
        ThÃƒÂªm m?t s? ki?n r?i ro vÃƒÂ o phiÃƒÂªn.

        Returns:
            True n?u phiÃƒÂªn b? quarantine, False n?u v?n bÃƒÂ¬nh thÃ†Â°Ã¡Â»Âng.

        FIX v3.6c (BUG-Decay Ã¢â‚¬- quarantine freeze):
            Tru?c d y khi quarantined=True, hÃƒÂ m return True ngay mÃƒÂ  khÃƒÂ´ng g?i
            _apply_decay() ? last_activity khÃƒÂ´ng bao gi? Ã„â€˜Ã†Â°Ã¡Â»Â£c c?p nh?t ?
            cumulative_score dÃƒÂ³ng bang ? m?c quarantine, khÃƒÂ´ng th? gi?m.
            Fix: g?i _apply_decay() tru?c d? decay score vÃƒÂ  c?p nh?t last_activity
            ngay c? khi session dÃƒÂ£ quarantined.

        [FIX ISSUE-7] Khi quarantined=True vÃƒÂ  hÃƒÂ m return s?m, last_activity
            khÃƒÂ´ng Ã„â€˜Ã†Â°Ã¡Â»Â£c c?p nh?t. ÃƒÂi?u nÃƒÂ y khi?n elapsed time tÃƒÂ­ch luy vÃƒÂ´ h?n
            ? decay_factor ? 0 ? score ? 0 sau nhi?u l?n g?i, nhung quarantined
            v?n lÃƒÂ  True vinh vi?n (khÃƒÂ´ng auto-recover).
            ÃƒÂ y lÃƒÂ  INTENTIONAL: quarantine lÃƒÂ  permanent cho d?n khi reset() Ã„â€˜Ã†Â°Ã¡Â»Â£c g?i.
            last_activity Ã„â€˜Ã†Â°Ã¡Â»Â£c c?p nh?t sau _apply_decay() d? gi? decay chÃƒÂ­nh xÃƒÂ¡c
            ngay c? trong tr?ng thÃƒÂ¡i quarantined.
        """
        # [FIX v3.6c] Decay TRU?C khi ki?m tra quarantine d? last_activity
        # luÃƒÂ´n Ã„â€˜Ã†Â°Ã¡Â»Â£c c?p nh?t vÃƒÂ  score khÃƒÂ´ng b? dÃƒÂ³ng bang.
        ...
        if self.quarantined:
            ...
            # d? decay ti?p t?c chÃƒÂ­nh xÃƒÂ¡c khi session Ã„â€˜Ã†Â°Ã¡Â»Â£c inspect sau quarantine.
    ...
    def get_risk_summary(self) -> Dict:
        """Tr? v? tÃƒÂ³m t?t r?i ro c?a phiÃƒÂªn."""
        return {
...
class ActionChainDetector:
    """
    PhÃƒÂ¡t hi?n chu?i hÃƒÂ nh Ã„â€˜Ã¡Â»â„¢ng nguy hiÃ¡Â»Æ’m (attack chaining).

    FIX v3.6b:
      - detect_chains(): trÃƒÂ¡nh duplicate matches cho cÃƒÂ¹ng m?t rule b?ng cÃƒÂ¡ch
        break sau khi tÃƒÂ¬m th?y following match d?u tiÃƒÂªn cho m?i preceding action.
        (Tru?c d y ti?p t?c vÃƒÂ²ng l?p ? cÃƒÂ¹ng m?t c?p cÃƒÂ³ th? b? report nhi?u l?n)
      - reset(): clear c? _seen_pairs d? trÃƒÂ¡nh stale state gi?a cÃƒÂ¡c session.
      - _seen_pairs: dedup guard d?a trÃƒÂªn (rule_name, preceding_action, following_action)
        d? trÃƒÂ¡nh re-report combo dÃƒÂ£ bi?t trong cÃƒÂ¹ng session.
    """
    ...
    action_history_window: int = 300  # 5 phÃƒÂºt
    ...
    def __post_init__(self):
        self.chain_rules = [
            ActionChainRule(
                ...
                risk_score=0.95,
                description="ÃƒÂ?c file nh?y c?m + g?i d? li?u ngoÃƒÂ i = Exfiltration",
            ),
            ActionChainRule(
            ...
            ActionChainRule(
                ...
                risk_score=0.85,
                description="TÃƒÂ¬m ki?m + XÃƒÂ³a b?ng ch?ng = Cover tracks",
            ),
            ActionChainRule(
    ...
    def record_action(self, action: str, sim_time: Optional[datetime] = None) -> None:
        """Ghi l?i hÃƒÂ nh Ã„â€˜Ã¡Â»â„¢ng m?i, trim hÃƒÂ nh Ã„â€˜Ã¡Â»â„¢ng quÃƒÂ¡ cu.
    ...
    def detect_chains(self) -> List[Dict]:
        """
        PhÃƒÂ¡t hi?n chu?i hÃƒÂ nh Ã„â€˜Ã¡Â»â„¢ng nguy hiÃ¡Â»Æ’m.

        FIX v3.6b:
          - Break ngay sau khi tÃƒÂ¬m Ã„â€˜Ã†Â°Ã¡Â»Â£c following match d?u tiÃƒÂªn cho m?i
            (rule, preceding) d? trÃƒÂ¡nh duplicate report.
          - DÃƒÂ¹ng _seen_pairs d? khÃƒÂ´ng report l?i combo dÃƒÂ£ bi?t trong session.
        """
        detected = []
        ...
        for rule in self.chain_rules:
            for i, (ts_pre, action_pre) in enumerate(self.recent_actions):
                if not re.search(rule.preceding_pattern, action_pre, re.IGNORECASE):
                    continue
                # FIX: early-exit Ã¢â‚¬- ch? l?y following match d?u tiÃƒÂªn
                for j in range(i + 1, len(self.recent_actions)):
                    ...
                    if re.search(rule.following_pattern, action_follow, re.IGNORECASE):
                        ...
                        break  # FIX: khÃƒÂ´ng ti?p t?c tÃƒÂ¬m following khÃƒÂ¡c cho cÃƒÂ¹ng preceding
    ...
    def reset(self) -> None:
        """FIX v3.6b: clear c? recent_actions, detected_chains, vÃƒÂ  _seen_pairs."""
...
def main():
    ...
    print("\n>> Running comparison on Nigga Dataset (Escalation Rate: " + str(session_rate) + ")...\n")
    summary_df = run_comparison_improved(dataset, target_escalation_rate=0.10)
    print("=" * 80)
    print("  BENCHMARK RESULTS ON NIGGA DATASET")
    print("=" * 80)
