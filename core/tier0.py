...
_RAW_PATTERNS: List[Tuple[str, ScanDecision, str]] = [
    ...
    # ── Neuralchemy Evasion & Jailbreak Patterns ──────────────────
    (r'(?i)\b(?:I\s+have\s+been\s+|Ive\s*ben\s*)?PWNED\b',
     ScanDecision.BLOCK, 'neuralchemy_pwned_jailbreak'),
    (r'(?i)\bwhoam[iι]\b',
     ScanDecision.BLOCK, 'recon_whoami_execution'),
    (r'(?i)\|\s*zstd\s+-[zd]',
     ScanDecision.BLOCK, 'evasion_zstd_compression_pipe'),
    (r'(?i)\bnim\s+-e\s+.*?\bexec\s*\(',
     ScanDecision.BLOCK, 'rce_nim_eval'),
    ...
    (r'(?i)\b(?:safe\s+email\s+list|blacklist)\s*:\s*[a-zA-Z0-9_.+-]+[@a]',
    ...
    (r'(?i)\bdo\s+this\s+before\s+you\s+summarize\b',
     ScanDecision.BLOCK, 'task_sandwich_injection'),
    (r'(?i)\bProject\s+Zenith\b.{0,150}?\b(?:send\s+(?:an?\s+)?email|contact[@a])\b',
