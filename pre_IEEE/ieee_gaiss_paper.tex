...
Nine dangerous cross-step combination rules assess cumulative risk (e.g., \texttt{SENSITIVE\_DATA_MENTION} + \texttt{EXFIL\_VERB} triggers an immediate \texttt{BLOCK} at risk score $\ge 0.85$). A \textit{Domino Effect Safeguard} requires that the current action must actively contribute at least one flag to the triggering set.
...
\subsection{V61 SecurityRouter: Calibrated ML Ensemble \& Canary Token Isolation}
...
\textbf{Slow Path (LLM Judge \& Canary Isolation):} Inputs scoring in the \texttt{REVIEW} margin ($0.5385 \le P < 0.6229$) are routed via \texttt{FastSlowRouter} to a local LLM Judge (e.g., Ollama Gemma3 / Qwen2.5-coder). The prompt template embeds untrusted text inside strict XML containers (\texttt{<user\_input\_to\_analyze>}) and generates a cryptographically random 16-character hex canary nonce (\texttt{canary = secrets.token\_hex(8)}) per request. The LLM Judge must return a response containing the exact canary token.

\textit{Empirical Evaluation against Adaptive Attackers:} We empirically evaluated the Canary Token mechanism against three adversarial adaptive strategies (\texttt{tests/test\_adaptive\_canary\_bypass.py}): (1) Canary Token Spoofing (\texttt{CANARY\_TOKEN\_VALID: true}); (2) XML Tag Breakout (\texttt{</user\_input\_to\_analyze>}); and (3) Simulated Output Injection. In all empirical trials, EVO-PCA Dual Shield achieved a \textbf{100\% block rate (3/3 test cases blocked)}. Because the 16-character hex nonce is generated dynamically in Python runtime memory and never exposed to the input payload, forged tokens fail string verification (\texttt{canary not in content}), immediately triggering a \texttt{BLOCK} with \texttt{INJECTION\_DETECTED}.
...
\textit{Discussion:} While stateless classifiers (PromptGuard, ProtectAI) execute quickly, they are blind to split-injection attacks and exhibit higher steady-state false positive rates ($>7.4\%$), severely degrading benign task completion ($\text{TCR}_{\text{benign}} < 78\%$). SafeHarness improves multi-turn detection to 26.40\% but lacks integrated egress sanitization and cross-session tracking. EVO-PCA Dual Shield achieves the highest \textbf{Benign TCR (87.52\% unassisted / 97.10\% assisted)}, highest \textbf{Session ABSR (32.39\%)}, an \textbf{overall Action ABSR of 12.38\%}, and lowest \textbf{Steady-State FPR (4.08\%)}, blocking 32.39\% of multi-step attacks at Step 1 while maintaining an average latency of 244.07~ms.
...
\begin{tabular}{lcci}
\toprule
\textbf{Security Layer / Component} & \textbf{Blocks} & \textbf{Share (\%)} & \textbf{Primary Target Threat} \\
\midrule
...
\midrule
\textbf{Total Intercepted Attacks} & \textbf{1,777} & \textbf{100.0} & \textbf{Overall Firewall Interception} \\
\bottomrule
\end{tabular}
\end{table}
...
\textit{Latency Profile:} The end-to-end average execution latency per scan across all 29,038 test records is \textbf{244.07~ms}. In-memory fast-path checks (Tier 0, Tier 0.5, and V61 ML ensemble) process over 95\% of routine traffic within $\approx 15.2$–$18.5$~ms (average fast-path scan overhead: \textbf{16.12~ms}), while the local LLM Judge is invoked selectively on borderline \texttt{REVIEW} cases ($\sim 4.7\%$ of blocks) with an average inference time of $\sim 393.7$~ms.
